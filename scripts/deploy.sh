...
# Reads PI_HOST, PI_USER, PI_SSH_KEY from env; falls back to generic defaults.
# Per-machine values belong in your shell profile (see DEPLOYMENT.md).

set -e

PI_HOST="${PI_HOST:-retropie.local}"
PI_USER="${PI_USER:-pi}"
SSH_KEY="${PI_SSH_KEY:-$HOME/.ssh/pi_key}"
